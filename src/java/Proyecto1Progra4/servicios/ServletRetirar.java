    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(numCuenta != null && !numCuenta.isEmpty() && (monto > 0) && fecha != null && (aplicado == 1 || aplicado == 0) &&
            ...
            if(saldoActual > monto){
                ...
                RequestDispatcher dispatcher = request.getRequestDispatcher("/MsjExito/RetiroExitoso.jsp");  
                dispatcher.forward(request, response);
            }else{
                request.setAttribute("saldoActual", saldoActual);
                RequestDispatcher dispatcher = request.getRequestDispatcher("/Errores/MontoMenorASaldo.jsp");  
                dispatcher.forward(request, response);
            }  
        }else {
            RequestDispatcher dispatcher = request.getRequestDispatcher("/Errores/DatosIncompletosRetiro.jsp");  
            dispatcher.forward(request, response);
        }
    }
